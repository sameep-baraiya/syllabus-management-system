...
// TODO Fix
const FindAcademicBatch = ({ defaultSelect = 'all' }) => {
  const academicBatchContext = useContext(AcademicBatchContext);
  const { getAcademicBatches, clearAcademicBatches } = academicBatchContext;
  ...
  const selectionClause = (e) => {
    if (e.target.name === 'select.all') {
      if (e.target.checked === true) {
        ...
        setAdvancedSearchQuery({
          ...advancedSearchQuery,
          select: 'id,academicBatchCode,academicBatchName',
        });
      }
    } else {
  ...
  const advancedSearch = () => (
    <Form.Group
      as={Row}
      controlId='FindAcademicBatch.advancedAcademicBatchSearch'
    >
      <Col sm={4}>
        ...
        {advancedSearchQuery.select !== 'all' ? (
          <Fragment>
            ...
            />
            <Form.Check
              name='select.academicBatchCode'
              type='checkbox'
              label='Academic Batch Code'
              defaultChecked
              disabled
            />
            <Form.Check
              name='select.academicBatchName'
              type='checkbox'
            ...
            />
            <Form.Check
              name='select.academicBatchDescription'
              type='checkbox'
              label='Academic Batch Description'
              onChange={onChange}
            />
            <Form.Check
              name='select.startYear'
              type='checkbox'
              label='Start Year'
              onChange={onChange}
            />
      ...
      <Col sm={4}>
        ...
        <Form.Control name='sortBy' as='select' onChange={onChange}>
          ...
          <option value='academicBatchCode'>Academic Batch Code</option>
          <option value='academicBatchName'>Academic Batch Name</option>
  ...
  return (
    <Form onSubmit={onSearch}>
      <Form.Group controlId='FindAcademicBatch.academicBatchSearch'>
        <h3>
          <strong>{iconSearch} Search Academic Batch</strong>
        </h3>
        ...
        <InputGroup className='mb-3'>
          <FormControl
            name='searchQuery'
            type='text'
            placeholder='Search by Academic Batch Code, Academic Batch Name'
            value={search}
...
FindAcademicBatch.propTypes = {
  defaultSelect: PropTypes.string.isRequired,
};

export default FindAcademicBatch;
